...
class Console(View):
    """Данный класс реализует взаимодействие с пользователем в консольном приложении"""
    __working = False
    __save = True
    __open = False
    ...
    def show_all(self):
        """Вывод всех заметок в консоль в виде таблицы"""
        if self.presenter.is_full():
            print("\n\t\t\t\t\t\t\t\t\tСПИСОК ВСЕХ ЗАМЕТОК",
    ...
    def show_filtered_notebook(self):
        """Вывод заметок на экран в виде таблицы, отфильтрованных по дате"""
        if self.presenter.is_full():
    ...
    def remove_note(self):
        """Метод для удаления заметки по номеру-идентификатору"""
        if self.presenter.is_full():
    ...
    def change_note(self):
        """Метод для изменения заметки по номеру-идентификатору"""
        if self.presenter.is_full():
            index = self.__get_index(self.presenter.get_size_notebook(),
                                     "\nВведите номер заметки: ")
